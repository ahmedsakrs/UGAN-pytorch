...
parser.add_argument("--g_checkpoint", type=str, required=False)
parser.add_argument("--d_checkpoint", type=str, required=False)
...
parser.add_argument(
    "--lbda2", type=int, default=1, help="weight for iamge gradient difference loss"
)
parser.add_argument(
    "--num_workers", type=int, default=4, help="Use multiple kernels to load dataset"
)
parser.add_argument(
    "--checkpoints_root",
    type=str,
...
parser.add_argument(
    ...
    type=str,
    default="0",
    help="Choose one gpu to use. Only single gpu training is supported currently",
)
...
if __name__ == "__main__":
    ...
    if args.g_checkpoint and args.d_checkpoint:
        netG.load_state_dict(torch.load(args.g_checkpoint))
        netD.load_state_dict(torch.load(args.d_checkpoint))
    ...
    for epoch in range(0, max_epoch):
        ...
        for iteration, data in enumerate(datasetloader):
            ...
            for i in range(num_critic):
                ...
                d_loss = torch.mean(d_fake) - torch.mean(
                    d_real
                )  # E[D(I_C)] = E[D(G(I_D))]
            ...
            writer.add_scalar(
                "G_loss", g_loss_log, (epoch * len(datasetloader) + iteration)
            )
            writer.add_scalar(
                "D_loss", d_loss_log, (epoch * len(datasetloader) + iteration)
            )
            writer.add_scalar(
                "base_loss", base_loss_log, (epoch * len(datasetloader) + iteration)
            )
            writer.add_scalar(
                "l1_loss", l1_loss_log, (epoch * len(datasetloader) + iteration)
            )
            writer.add_scalar(
                "IGDL_loss", igdl_loss_log, (epoch * len(datasetloader) + iteration)
            )
            print(
                "==>Epoch:%d/%d:%d d_loss:%.3f g_loss:%.3f"
                % (epoch, max_epoch, iteration, d_loss_log, g_loss_log)
            )
        ...
        torch.save(
            netD.state_dict(), os.path.join(checkpoint_dir, "netD_%d.pth" % epoch)
        )
        torch.save(
            netG.state_dict(), os.path.join(checkpoint_dir, "netG_%d.pth" % epoch)
        )
